...
const RouteMap = forwardRef<RouteMapRef, RouteMapProps>(({ 
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Store route line reference for hover interactions
      routeLineRef.current = routeLine;

      // Add hover interaction to route line
      routeLine.on('mousemove', (e: any) => {
        ...
        gpxData.points.forEach((point, index) => {
          const distance = Math.sqrt(
            Math.pow(point.lat - latlng.lat, 2) + Math.pow(point.lon - latlng.lng, 2)
          );
